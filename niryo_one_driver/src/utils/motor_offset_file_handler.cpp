...
bool niryo_one_hardware::get_motors_calibration_offsets(std::vector<int> &motor_id_list,  std::vector<int> &steps_list)
{
    ...
    if (offset_file.is_open()) {
        ...
        RCLCPP_WARN(rclcpp::get_logger("Motor Calibration Offsets"),"Unable to open file : %s", file_name.c_str());
        return false;
    }
...
bool niryo_one_hardware::set_motors_calibration_offsets(std::vector<int> &motor_id_list, std::vector<int> &steps_list)
{
...
/*
    ...
    if (returned_error) {
        RCLCPP_WARN(rclcpp::get_logger("Motor Calibration Offsets"),"Could not create directory : %s", folder_name.c_str()); 
        return false;
    }
*/
    ...
    if (offset_file.is_open()) {
        RCLCPP_INFO(rclcpp::get_logger("Motor Calibration Offsets"),"Writing calibration offsets to file : \n%s", text_to_write.c_str());
